...
#definitions (kept for backward compatibility, actual definitions are in perma_calc_core):
...
UW_CONFIG = config.UW_PERMA_CONFIG

# UW order for consistent display
UW_ORDER = ["Chrono Field", "Black Hole", "Golden Tower", "Death Wave", "Golden Bot", "Summon Guardian", "Smart Missiles", "Inner Land Mines", "Poison Swamp"]
...
def is_package(
    ...
    Notes:
      - On boss waves: if boss packages are enabled, ONLY the boss package can drop (no chance-based package)
      - On boss waves: if boss packages are disabled, normal wave package chance applies
      - On regular waves: only chance-based packages can drop
    ...
    # Check if this is a boss wave (every boss_every_x waves)
    is_boss_wave = time % boss_time < 1
    
    if is_boss_wave and boss_package_enabled:
        # Boss wave with boss packages enabled: guaranteed boss package, no random chance
        package_reduction = pkg_reduction
        is_boss_package = True
    elif time % wave_time < 1:
        # Regular wave OR boss wave with boss packages disabled: apply chance-based package
...
def calculate_uw_uptime(
    ...
    for t in timeline:
        ...
        # Step 4: Record state at this second
        # If activation occurred, record cooldown as 0 to show it reached the trigger point
        # Otherwise record the actual remaining cooldown (clamped to 0 if negative due to overshoot)
    ...
    [
                        ...
                        [
                            dbc.Col(
                                [
                                    ...
                                    dbc.Input(id="uw-bh-cooldown", type="number", value=UW_CONFIG["Black Hole"]["cooldown"], step=1),
                                ],
                                width=2,
                            ),
                            dbc.Col(
                                [
                                    ...
                                    dbc.Input(id="uw-bh-duration", type="number", value=UW_CONFIG["Black Hole"]["duration"], step=1),
                                ],
                                width=2,
                            ),
                            dbc.Col(
                                [
                                    ...
                                    dbc.Input(id="uw-gt-cooldown", type="number", value=UW_CONFIG["Golden Tower"]["cooldown"], step=1),
                                ],
                                width=2,
                            ),
                            dbc.Col(
                                [
                                    ...
                                    dbc.Input(id="uw-gt-duration", type="number", value=UW_CONFIG["Golden Tower"]["duration"], step=1),
                                ],
                                width=2,
                            ),
                            dbc.Col(
                                [
                                    ...
                                    dbc.Input(id="uw-dw-cooldown", type="number", value=UW_CONFIG["Death Wave"]["cooldown"], step=1),
                                ],
                                width=2,
                            ),
                            dbc.Col(
                                [
                                    ...
                                    dbc.Input(id="uw-dw-duration", type="number", value=UW_CONFIG["Death Wave"]["duration"], step=1),
                                ],
                                width=2,
                            ),
                        ],
                        ...
                        [
                            dbc.Col(
                                [
                                    ...
                                    dbc.Input(id="uw-cf-cooldown", type="number", value=UW_CONFIG["Chrono Field"]["cooldown"], step=1),
                                ],
                                width=2,
                            ),
                            dbc.Col(
                                [
                                    ...
                                    dbc.Input(id="uw-cf-duration", type="number", value=UW_CONFIG["Chrono Field"]["duration"], step=1),
                                ],
                                width=2,
                            ),
                            dbc.Col(
                                [
                                    ...
                                    dbc.Input(id="uw-gb-cooldown", type="number", value=UW_CONFIG["Golden Bot"]["cooldown"], step=0.5),
                                ],
                                width=2,
                            ),
                            dbc.Col(
                                [
                                    ...
                                    dbc.Input(id="uw-gb-duration", type="number", value=UW_CONFIG["Golden Bot"]["duration"], step=1),
                                ],
                                width=2,
                            ),
                               dbc.Col(
                                   [
                                       dbc.Label("Summon Guardian Cooldown (s)"),
                                       dbc.Input(id="uw-sg-cooldown", type="number", value=UW_CONFIG["Summon Guardian"]["cooldown"], step=1),
                                   ],
                                   width=2,
                               ),
                               dbc.Col(
                                   [
                                       dbc.Label("Summon Guardian Duration (s)"),
                                       dbc.Input(id="uw-sg-duration", type="number", value=UW_CONFIG["Summon Guardian"]["duration"], step=1),
                                   ],
                                   width=2,
                               ),
                            dbc.Col(
                                [
                                    ...
                                    dbc.Input(id="uw-sm-cooldown", type="number", value=UW_CONFIG["Smart Missiles"]["cooldown"], step=1),
                                ],
                                width=2,
                            ),
                            dbc.Col(
                                [
                                    ...
                                    dbc.Input(id="uw-sm-duration", type="number", value=UW_CONFIG["Smart Missiles"]["duration"], step=1),
                                ],
                                width=2,
                            ),
                        ],
                        ...
                        [
                            dbc.Col(
                                [
                                    ...
                                    dbc.Input(id="uw-ilm-cooldown", type="number", value=UW_CONFIG["Inner Land Mines"]["cooldown"], step=1),
                                ],
                                width=2,
                            ),
                            dbc.Col(
                                [
                                    ...
                                    dbc.Input(id="uw-ilm-duration", type="number", value=UW_CONFIG["Inner Land Mines"]["duration"], step=1),
                                ],
                                width=2,
                            ),
                            dbc.Col(
                                [
                                    ...
                                    dbc.Input(id="uw-ps-cooldown", type="number", value=UW_CONFIG["Poison Swamp"]["cooldown"], step=1),
                                ],
                                width=2,
                            ),
                            dbc.Col(
                                [
                                    ...
                                    dbc.Input(id="uw-ps-duration", type="number", value=UW_CONFIG["Poison Swamp"]["duration"], step=1),
                                ],
                                width=2,
                            ),
                        ],
        ...
        dbc.Row(
            [
                ...
                dbc.Col([
                    dbc.Card([dbc.CardBody([
                        html.H6('Summon Guardian', className='card-title', style={'marginBottom': '0.5rem'}),
                        html.Div(id='stats-sg-params', style={'color': '#6c757d', 'fontSize': '0.8rem', 'marginBottom': '0.5rem'}),
                        html.Div(id='stats-sg-uptime', style={'fontSize': '1rem', 'marginBottom': '0.25rem'}),
                        html.Div(id='stats-sg-downtime', style={'color': '#999', 'fontSize': '0.85rem', 'marginBottom': '0.25rem'}),
                        html.Div(id='stats-sg-interval', style={'color': '#999', 'fontSize': '0.85rem'}),
                    ])], id='stats-sg-card', className='mb-2'),
                ], width=12, md=6, lg=4),
                dbc.Col([
                    dbc.Card([dbc.CardBody([
...
@callback(
    ...
    Input("uw-sg-cooldown", "value"),
    Input("uw-sg-duration", "value"),
...
def calculate_simulations(
    ...
    # perks_on is True if the checklist is non-empty and we're in Farming mode
    perks_on = play_mode == "Farming" and bool(perk_status)
    ...
    ("Summon Guardian", (sg_cooldown or 100) + bc_cooldown, sg_duration or 30, 0, False),  # Like Golden Bot
    ...
    for i, uw_name in enumerate(UW_ORDER):
        ...
        base_cd = [
            cf_cooldown or 180,  # Chrono Field
            bh_cooldown or 46,   # Black Hole
            gt_cooldown or 170,  # Golden Tower
            dw_cooldown or 170,  # Death Wave
            gb_cooldown or 100,  # Golden Bot
            sg_cooldown or 100,  # Summon Guardian
            sm_cooldown or 120,  # Smart Missiles
            ilm_cooldown or 130, # Inner Land Mines
            ps_cooldown or 140   # Poison Swamp
        ][i]
        base_dur = [
            cf_duration or 28,   # Chrono Field
            bh_duration or 34,   # Black Hole
            gt_duration or 45,   # Golden Tower
            dw_duration or 20,   # Death Wave
            gb_duration or 24.5, # Golden Bot
            sg_duration or 30,   # Summon Guardian
            sm_duration or 15,   # Smart Missiles
            ilm_duration or 25,  # Inner Land Mines
            ps_duration or 30    # Poison Swamp
        ][i]
        perk_dur = [
            cf_perk_duration,    # Chrono Field
            bh_perk_duration,    # Black Hole
            0,                  # Golden Tower
            dw_perk_duration,   # Death Wave
            0,                  # Golden Bot
            0,                  # Summon Guardian
            0,                  # Smart Missiles
            0,                  # Inner Land Mines
...
@callback(
    ...
    Output("stats-sg-params", "children"),
    Output("stats-sg-uptime", "children"),
    Output("stats-sg-downtime", "children"),
    Output("stats-sg-interval", "children"),
    Output("stats-sg-card", "style"),
