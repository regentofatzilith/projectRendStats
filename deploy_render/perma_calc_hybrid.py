...
UW_CONFIG_DF = pd.DataFrame([
    {"name": name, "base_cooldown": cfg["cooldown"], "base_duration": cfg["duration"], "color_hex": cfg["color_hex"], "can_queue": cfg["can_queue"]}
    for name, cfg in config.UW_PERMA_CONFIG.items()
])
